...
4. Recursion direction matters
  ...
  which traverses from lower digits to higher ones.

5. Faster solution exploiting the structure of linked list!
...
6. As the complexity grows, Dafny's performance becomes unpredictable!!
  ...
  For example, before introducing `addTwoNumbers'` (see Discussion 5), everything passed under 1 second.
  However, I had to make the `partial_sum` function opaque, in order to make `addTwoNumbers'` pass.
  This mysteriously slows down the original `addTwoNumbers` by ~10x.

  Although the whole thing is now verified in under 5 seconds (by Dafny 4.3.0.0),
  whether and where to place what assertions/lemmas is all "results of trial and error".
  It may even fail in another Dafny version!!

  More reference: http://dafny.org/dafny/VerificationOptimization/VerificationOptimization
  {:vcs_split_on_every_assert} indeed does some (limited) magic, and helps a bit on troubleshooting...

7. Fun duality! ...Between program and proof
  We hear a lot about using theorems to prove a program, but how about using a program to prove a theorem?!
  At the end of this file, you see `lemma_quick_sum` does exactly this!
...
// Alternative definition of `value_of` (see Discussion 4)
...
// An optimizated solution that works better when `|l1|` and `|l2|` differs a lot (see Discussion 5)
method {:vcs_split_on_every_assert} addTwoNumbers'(l1: seq<digit>, l2: seq<digit>) returns (sum: seq<digit>)
  requires valid_seq(l1) && valid_seq(l2)
  ensures valid_seq(sum)
  ensures value_of(sum) == value_of(l1) + value_of(l2)
